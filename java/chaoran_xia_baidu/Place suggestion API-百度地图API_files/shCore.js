		...
		func: function(_8, _9) {
			...
			if (window.clipboardData) {
				...
				if (dp.sh.ClipboardSwf != null) {
					...
					_b.innerHTML = "<embed src=\""
							+ dp.sh.ClipboardSwf
							+ "\" FlashVars=\"clipboard="
							+ encodeURIComponent(_a)
...
dp.sh.Toolbar.Create = function(_13) {
	...
	for (var _15 in dp.sh.Toolbar.Commands) {
		...
		var html = "<a href=\"#\" onclick=\"dp.sh.Toolbar.Command('" + _15 + "',this);return false;\"";
		if (_15 == 'CopyToClipboard') {
			var id = 'clip' + parseInt(Math.random() * 1000);
			html += " id=\"" + id + "\"";
			setTimeout(function() {
				...
				clip.addEventListener('mouseOver', function(client) {
					...
					var code = div.parentNode.highlighter.code.replace(/&lt;/g, '<').replace(/&gt;/g, '>').replace(/&quot;/, '"')
					clip.setText(code);
				});
				clip.addEventListener('complete', function(client, text) {
					alert("代码目前已经添加到您的剪切板");
				});
				clip.glue(id);
			}, 1000);
		}
...
dp.sh.Highlighter.prototype.GetMatches = function(_25, css) {
	...
	while ((_28 = _25.exec(this.code)) != null) {
		this.matches[this.matches.length] = new dp.sh.Match(_28[0], _28.index,
			css);
	}
};
...
dp.sh.HighlightAll = function(_5f, _60, _61, _62, _63, _64) {
	...
	for (var i = 0; i < _75.length; i++) {
		var _7d = _75[i];
		var _7e = FindValue(_7d.attributes["class"], _7d.className,
			_7d.attributes["language"], _7d.language);
		var _7f = "";
		if (_7e == null) {
			continue;
		}
		...
		_76.firstLine = (_63 == null) ? parseInt(GetOptionValue("firstline",
			_7e, 1)) : _63;
