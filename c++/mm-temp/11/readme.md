...
```
sort(beg, end);           // use < operator to sort the element
sort(beg, end, comp);     // use function named comp to sort the element
```

检查指定值的算法默认使用`==`操作符。系统为这类算法提供了另外命名的（而非重载）版本，带有谓词函数形参。带有谓词函数形参的算法，其名字后带有后缀`_if`:
```
find(beg, end, val);      // find first instance of val in the input range
find(beg, end, pred);     // find first instance for which pred is true
```
上述两个算法都在输入范围内寻找指定元素的第一个实例。其中，`find`算法查找一个指定的值，而`find_if`算法则用于查找一个使谓词函数`pred`返回非零值的元素。

对于查找指定元素的算法，不管检查的是一个值还是谓词函数，函数调用都需要相同个数的参数。此时，如果使用重载版本，则可能导致二义性，尽管这个可能出现的几率较低。因此，标准库为这些算法提供两种不同名字的版本，而没有使用重载。
...
### 11.5 容器特有的算法
`list`容器上的迭代器是双向的，而不是随机访问类型。由于`list`容器不支持随机访问，因此，在此容器上不能使用需要随机访问迭代器的算法。这些算法包括`sort`及其相关的算法。还有一些其他的泛型算法，如`merge`，`remove`，`reverse`和`unique`，虽然可以用在`list`上，但却付出了性能上的代价。如果这些算法利用`list`容器实现的特点，则可以更高效地操作。

如果可以结合利用`list`容器内部结构，则可能编写出更快的算法。与其他顺序容器所支持的操作相比，标准库为`list`容器定义了更加精细的操作集合，使它不必只依赖于泛型操作。

> 对于`list`对象，应该优先使用`list`容器特有的成员版本，而不是泛型算法；
> 与对应的泛型算法不同，`list`容器特有的容器能添加和删除元素；
=======
### 11.5 容器特有的算法
